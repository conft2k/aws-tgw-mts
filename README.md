...
원래 VPC 라우팅 테이블은 사람이(또는 API 호출로) 직접 써넣는 정적인 테이블입니다.
...
- **장중 무중단 요구에 실측 수치로 답할 수 있음** — 계획 전환·복귀는 무손실,
  비계획 장애도 BFD 감지(~1초)가 상한입니다. "장중에 라우터를 교체해도
  되는가" 같은 운영 질문에 측정값으로 답할 수 있습니다.
...
죽는 "친절한" 장애입니다. 커널 패닉, 하이퍼바이저 장애, 인스턴스 강제 종료처럼 아무
신호 없이 죽는 급사는 아래 BFD/BGP 타이머들이 감지를 담당합니다.
...
- **급사 시 기대치**: RS BFD 감지(0.9초) + 경로 교체 ≈ 순방향 **~1초 단절**
  (단방향 스트림 기준). 리턴 경로는 대기 라우터의 오버레이 BGP 수립까지
  수 초가 더 걸립니다.
...
iperf2 서버 리포트가 그대로 손실 측정 도구입니다 — 인터벌마다
`Lost/Total Datagrams` 열이 찍히므로 (초당 109패킷 = 해상도 ~9ms),
절체 순간의 손실이 0인지 몇 개인지 별도 캡처 없이 확인됩니다.
스트림 종료 시 전체 요약(총 datagram·손실률·평균 지터)도 출력됩니다.

수신 공백을 밀리초 단위로 더 정밀하게 봐야 하면 tcpdump 타임스탬프를
병행 기록해 최대 간격을 계산할 수 있습니다:
...
`iperf -c 239.1.1.1 -u -T 8 -b 1M -l 1200`, 초당 109패킷)을 흘리는 상태로
활성 C8000V 인스턴스를 **중지 → 재시작**. 리시버 2대의 iperf2 서버
(`iperf -s -u -B 239.1.1.1 -i 1`)가 초 단위로 손실/지터를 직접 리포트합니다.
...
- 페일오버 시점의 라우트 교체 지연 편차(+2초 vs +18초)는 IOS-XE 셧다운 진행
  타이밍 차이일 뿐, 양쪽 모두 수신 공백은 없었습니다.
...
페일오버 구간 — 지터만 0.1ms대 → 0.95ms로 출렁 (손실 0):
...
페일백 구간 — 선점 전환이 지터 스파이크 몇 개로만 관측 (손실 0):
...
600초 스트림 최종 요약 — 페일오버·페일백을 모두 포함한 전 구간 손실 0:
...
- **장애·복귀 양방향 모두 make-before-break가 성립해 무손실.**
  중지 방향은 IOS-XE가 BGP를 정상 종료(graceful close)해 데이터플레인이 멎기
  전에 경로가 먼저 이동하고, 복귀 방향은 라우터가 완전히 부팅해 PIM·static-group
  까지 준비된 뒤에야 BGP가 올라와 경로가 이동하기 때문입니다.
- **순방향 멀티캐스트는 오버레이 BGP 재수렴과 무관** — Gi2의
  `igmp static-group` 사전 (*,G)와 정적 mroute RPF 덕에 경로 전환 즉시 흐릅니다.
  단방향 배포형 워크로드(시세/영상 피드)는 전 과정 무손실.
- **graceful close가 없는 급사**(커널 패닉, 강제 종료)라면 RS BFD 감지(~1초)가
  공백 상한입니다 (4.4 타이머 설계 참고).
- 복구 후 원상 복귀는 자동(선점) — C8000V-1이 부팅하면 별도 작업 없이
  활성으로 돌아오며, 복귀 중에도 손실이 없으므로 운영 개입이 필요 없습니다.
- C8000V 부팅(start → RS BGP 재수립)은 **약 3분~3분 30초**.
