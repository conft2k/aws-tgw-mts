...
| **Persist Routes** | 피어 플랩 시 마지막 경로를 유지해 블랙홀 방지 | 재부팅 3회 반복에서 불필요한 경로 전환 없음 |
...
페일오버 실측(6.1)의 재부팅은 IOS-XE가 BGP를 정상 종료(graceful close)하고 죽는
"친절한" 장애입니다. 커널 패닉, 하이퍼바이저 장애, 인스턴스 강제 종료처럼 아무
...
- **급사 시 기대치**: RS BFD 감지(0.9초) + 경로 교체 ≈ 순방향 **~1초 단절**
  ...
  수 초가 더 걸립니다 (재부팅 실측 9.2초와 유사한 수준).
- **그레이스풀 종료 시**: BGP notification이 BFD 만료보다 먼저 도착해
  make-before-break가 성립 — 실측 무손실(6.1 단방향).
...
실측 예 (6.1 재부팅 테스트 전 구간): 리시버1 `max_gap=0.056s`, 리시버2 `max_gap=0.053s`
— 페일오버·페일백 포함 무손실.
...
| **순방향 멀티캐스트 손실** | **0건** (장애+복귀 전 구간, 0.1초 간격 5,114 패킷 검증) — Gi2 static-group의 사전 (*,G) 덕분 | **0초** (재부팅 테스트에서 전 구간 최대 간격 56ms = 지터 수준) |
| 리턴(응답) 경로 전환 | ≤1초 (오버레이 BFD + connect 5) | 해당 없음 (단방향 스트림은 리턴 경로 불필요) |
| 복귀(failback) 리턴 공백 | ~90초 — 부팅 직후 선점이 오버레이 BGP 준비보다 먼저 일어나는 구조적 한계. 복귀는 계획 작업으로 다루기를 권장 | **0초** (복귀 중에도 무손실 — 재부팅 테스트로 재확인) |
...
ping 통계도 맞아떨어집니다: 372 송신 / 손실 97 = 페일오버 8개 + 페일백 89개와
정확히 일치. 응답이 온 모든 seq는 리시버 2대 모두 응답했습니다(수신 275 + 중복 275).
