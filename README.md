...
### 4.4 타이머 설계 — 그레이스풀 종료와 급사 모두 대비

페일오버 실측(6.1)의 인스턴스 중지는 IOS-XE가 BGP를 정상 종료(graceful close)하고
죽는 "친절한" 장애입니다. 반면 커널 패닉, 하이퍼바이저 장애, 인스턴스 강제 종료처럼
아무 신호 없이 갑자기 죽는 경우는 아래 BFD/BGP 타이머들이 감지합니다.

| 타이머 | 값 | 감지 시간 | 담당 상황 |
|---|---|---|---|
| RS BFD (Gi1) | 300ms × 3 | ~0.9초 | **활성 라우터 급사** — RS가 경로를 철회해 페일오버를 트리거하는 핵심 타이머 |
...
- **급사 시 기대치**: RS BFD 감지(0.9초)에 경로 교체 시간을 더해 순방향 기준
  **약 1초의 단절**이 예상됩니다(단방향 스트림 기준). 리턴 경로는 대기 라우터의
  오버레이 BGP가 수립될 때까지 몇 초가 더 걸립니다.
- **그레이스풀 종료 시**: BGP notification이 BFD 만료보다 먼저 도착해
  make-before-break가 성립 — 실측 무손실(6.1 iperf2 실측).
...
- `-l 1200`: **데이터그램 크기 필수** — 기본 1470B는 터널 MTU(1476)를 넘어
  전량 소멸합니다 (7.2 참조). 1200B면 UDP/IP 포함 1228B로 여유
...
- **순방향 멀티캐스트는 오버레이 BGP 재수렴과 무관합니다** — Gi2의
  ...
  전 과정에서 손실이 없습니다.
- 커널 패닉이나 강제 종료처럼 **graceful close 없이 갑자기 죽는 경우**에는
  RS BFD가 장애를 감지하는 시간(~1초)까지만 수신이 끊길 수 있습니다
  (4.4 타이머 설계 참고).
