...
온프레미스의 멀티캐스트 트래픽을 AWS 리시버까지 전달하는 하이브리드 구성입니다.
경로는 세 단계입니다. 온프레미스 라우터(CGW)가 멀티캐스트를 **GRE 터널**에 실어
VPC 안의 라우터(C8000V)로 보내고, 그 라우터가 **TGW 멀티캐스트 도메인(IGMPv2)**에
트래픽을 내보내면, 도메인이 조인한 리시버들에게 복제해 전달합니다.
...
이 문서는 위 구성을 서울 리전에 실제로 배포하고, 활성 라우터를 반복적으로
죽여가며 측정한 결과를 담고 있습니다. 운영 관점의 핵심 질문과 실측 답:

| 질문 | 실측 답 |
|---|---|
| 라우터가 갑자기 죽으면 시세·영상 같은 단방향 스트림은 얼마나 끊기나? | **끊기지 않음** — 재부팅 반복 테스트에서 최대 수신 간격 56ms(지터 수준) |
| 요청-응답형(양방향) 트래픽은? | 장애 시 ~9초, 자동 복귀 시 ~90초 공백 |
| VPC 경로는 얼마나 빨리 전환되나? | BFD 감지 후 **~1초** (그레이스풀 종료 시 사실상 0) |
| 자동 복귀(페일백)는 안전한가? | 멀티캐스트는 무손실. 리턴 경로만 ~90초 공백이 나며 이는 구조적(튜닝 불가) → **복귀는 계획 작업으로** |
| 장애 때 온프레미스 라우터가 할 일은? | **없음** — 터널 목적지(애니캐스트 주소)가 변하지 않음 |

운영자가 기억할 세 가지:

1. **멀티캐스트 페이로드는 1400B 이하로** — 터널 MTU(1476)를 넘는 데이터그램은
   경고 없이 전량 소멸합니다 (ping은 통과해서 착각하기 쉬움, 5.2).
2. **리시버 멤버십은 조인 앵커 프로세스의 수명과 같습니다** — systemd로 유지하고,
   인스턴스가 교체되면 재설정해야 합니다 (5.3).
3. **페일백의 ~90초 리턴 공백은 제거할 수 없습니다** — 복귀는 장외 시간 등
   계획된 창에서 수행하세요 (4.1).
...
### 2.4 타이머 설계 — 그레이스풀 종료와 급사 모두 대비

페일오버 실측(4.1)의 재부팅은 IOS-XE가 BGP를 정상 종료(graceful close)하고 죽는
...
- **그레이스풀 종료 시**: BGP notification이 BFD 만료보다 먼저 도착해
  make-before-break가 성립 — 실측 무손실(4.1 단방향).
...
- `-l 1200`: **데이터그램 크기 필수** — 기본 1470B는 터널 MTU(1476)를 넘어
  전량 소멸합니다 (5.2 참조). 1200B면 UDP/IP 포함 1228B로 여유
