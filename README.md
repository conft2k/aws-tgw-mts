...
멎는, 말하자면 예고를 남기고 죽는 장애입니다. 반면 커널 패닉, 하이퍼바이저 장애,
인스턴스 강제 종료처럼 아무 신호 없이 멎는 장애는 아래 BFD/BGP 타이머들이
감지를 맡습니다.
...
- **갑자기 멎는 장애라면**: RS BFD 감지(0.9초)에 경로 교체 시간을 더해 순방향
...
- **장애와 복귀 양방향 모두 make-before-break가 성립해 무손실입니다.**
  ...
  멎기 전에 경로가 옮겨 가고, 복귀할 때는 라우터가 완전히 부팅해 PIM과
...
- **순방향 멀티캐스트는 오버레이 BGP 재수렴과 무관합니다** — Gi2의
  ...
  전 과정에서 손실이 없습니다.
- 커널 패닉이나 강제 종료처럼 **BGP를 정상 종료하지 못하고 갑자기 멎는
