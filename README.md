...
**1초 미만 장애 전환은 Amazon VPC Route Server로, 멀티캐스트 트래픽 복제는 AWS Transit Gateway로**

> English version: [README.en.md](README.en.md)

온프레미스 멀티캐스트를 AWS 리시버까지 끊김 없이 전달하는 하이브리드 구성을
서울 리전에 실제로 배포하고, 반복 장애 테스트로 검증한 기록입니다.

**목차**

- [1. 배경](#1-배경) — 문제, Route Server의 가치, 적용 환경, 아키텍처 개요
- [2. GRE 오버레이 연결 플로우](#2-gre-오버레이-연결-플로우)
- [3. 설정 순서](#3-설정-순서)
- [4. CloudFormation 배포 자원](#4-cloudformation-배포-자원)
- [5. 검증 가이드](#5-검증-가이드)
- [6. Failover 테스트](#6-failover-테스트) — 실측 결과
- [7. 모니터링 및 트러블슈팅](#7-모니터링-및-트러블슈팅)
- [Appendix. 라우터 상태 샘플 출력](#appendix-라우터-상태-샘플-출력-실측)
- [참조 (AWS 공식 문서)](#참조-aws-공식-문서)

## 1. 배경

### 문제: 하이브리드 멀티캐스트의 두 가지 벽

시세 분배, 방송 컨트리뷰션, 관제 시스템처럼 멀티캐스트에 의존하는 워크로드를
AWS로 확장하려 하면 두 가지 벽에 부딪힙니다.

1. **AWS 네트워크는 멀티캐스트를 기본적으로 전달하지 않습니다.** TGW 멀티캐스트
   도메인이 유일한 네이티브 수단이지만 이는 VPC 안에서만 동작하고,
   Direct Connect/VPN은 멀티캐스트를 전달하지 못하므로 온프레미스 PIM
   도메인과 직접 연동되지 않습니다. 그래서 **GRE 터널**로 멀티캐스트를
   VPC 안의 EC2 라우터(C8000V)까지 끌어온 뒤 TGW 도메인으로 내보내는
   우회 경로가 필요합니다.
...
옮겨 붙이거나, GWLB를 끼워 넣는 방식입니다. 어느 쪽이든 한계는 같습니다.
장애 감지에 수십 초에서 몇 분이 걸리고, 장애 판단과 전환 로직이 전부
커스텀 코드라 검증·유지보수 부담이 크며, 활성/대기 우선순위 같은 라우팅
정책을 표현할 표준적인 방법도 없습니다.

### VPC Route Server의 가치 — 이 구성의 핵심

원래 VPC 라우팅 테이블은 사람이 직접(또는 API 호출로) 써넣는 정적인 테이블입니다.
**Amazon VPC Route Server는 이 테이블이 BGP를 알아듣게 만들어 줍니다.** VPC 안의
라우터(EC2)가 BGP로 "이 목적지는 나한테 보내라"고 광고하면 그 경로가 라우팅
테이블에 자동으로 등록되고, 라우터가 죽으면(BFD 감지) 자동으로 빠집니다.
물리 네트워크에서 라우터끼리 늘 하던 일을 VPC 라우팅 테이블과도 하게 되는
것입니다. 이 구성에서 그 가치는:

| 가치 | 이 구성에서의 의미 | 실측 |
|---|---|---|
| **BFD 기반 1초 미만 장애 감지** | 커스텀 코드 없이 어플라이언스 상태 감시 | VPC 경로 전환 ~1초 |
...
| **Persist Routes** | 피어 플랩 시 마지막 경로를 유지해 블랙홀 방지 | 중지·재시작 반복에서 불필요한 경로 전환 없음 |
| **양쪽 라우팅 테이블 동시 주입** | tgw-rtb(GRE 유입)와 receiver-rtb(리턴 경로)를 한 세션으로 갱신 | 멀티캐스트 스트림 **무손실** (장애·복귀 전 과정) |

요약하면, 예전에는 Lambda로 직접 짜야 했던 페일오버 오케스트레이션이
BGP·BFD·prepend라는 30년간 검증된 프로토콜 동작으로 대체됩니다. 네트워크
엔지니어에게 익숙한 도구 그대로 VPC 라우팅을 다룰 수 있다는 것 — 그것이
이 구성이 성립하는 이유입니다.

### 어떤 환경에 효과적인가 — MTS 같은 실시간 시세 분배

이 아키텍처가 특히 효과적인 곳은 **증권사 MTS/HTS를 뒷받침하는 실시간 시세 분배**처럼,
온프레미스의 멀티캐스트 피드를 클라우드의 여러 수신 서비스로 끊김 없이 확장해야
하는 환경입니다.

- **트래픽 패턴이 정확히 일치** — 거래소 시세 피드는 UDP 멀티캐스트 **단방향
  스트림**입니다. 이 구성의 실측 결과(장애·복귀 전 과정 손실 0,
  6.1 iperf2 실측)가 그대로 적용되는 패턴입니다.
- **피드 소스를 건드릴 수 없음** — 시세 수신 회선·장비 구성은 규제와 계약상
  변경이 어렵습니다. 페일오버가 전적으로 AWS 쪽에서 일어나고 온프레미스(CGW)는
  무변경이라는 이 구성의 특성이 그 제약에 딱 들어맞습니다.
- **수신자는 클라우드에서 늘어남** — 시세를 소비하는 체결·호가·알림 서비스가
  AWS에서 수평 확장됩니다. TGW 도메인 덕에 수신자 추가는 IGMP 조인 하나로
  끝나고, 복제 부하는 TGW가 담당합니다 (라우터 부하 불변).
- **장중 무중단 요구에 실측 수치로 답할 수 있음** — 계획 전환과 복귀는 무손실이고,
  비계획 장애도 수신 공백이 BFD 감지 시간(~1초)을 넘지 않습니다. "장중에
  라우터를 교체해도 되는가" 같은 운영 질문에 측정값으로 답할 수 있습니다.

방송 컨트리뷰션 피드나 관제·텔레메트리 분배처럼 "단방향 멀티캐스트 + 소스
변경 불가 + 클라우드 쪽 수신자 확장" 조합이라면 같은 이유로 잘 맞습니다.

### 아키텍처 개요

트래픽 경로는 세 단계입니다. 온프레미스 라우터(CGW)가 멀티캐스트를 **GRE 터널**에
실어 VPC 안의 라우터(C8000V)로 보내고, 그 라우터가 **TGW 멀티캐스트
도메인(IGMPv2)**에 트래픽을 내보내면, 도메인이 조인한 리시버들에게 복제해
전달합니다.

고가용성의 핵심은 **애니캐스트**입니다. VPC 라우터 2대가 같은 터널 종단 주소
(공유 Loopback)를 갖고, 실제로 누가 터널을 받을지는 **VPC Route Server**가
결정합니다. 활성 라우터가 죽으면 Route Server가 BFD로 1초 미만에 감지해 경로를
대기 라우터로 바꾸고(우선순위는 AS-path prepend로 표현), 터널은 같은 주소를 가진
대기 라우터에 자동으로 다시 붙습니다. 온프레미스(CGW)는 터널 목적지가 그대로라
**아무것도 바꿀 필요가 없습니다**.
...
VPC 10.1.1.0/24
  ...
  │
  └─ [receiver-sub .64/26]
       C8000V Gi2 .90/.100 (igmp static-group) → TGW 멀티캐스트 도메인으로 송출
...
- **AWS → 온프렘**: CGW가 DX로 Lo0(`172.20.255.1/32`)만 광고(`LOOPBACK-ONLY`) → TGW에 전파.
  C8000V는 정적 경로 `172.20.255.1/32 → Gi1 게이트웨이` → router-rtb의 `/32 → TGW` 경로를 타고 되돌아갑니다
...
CGW: 원본 패킷을 GRE로 캡슐 (outer src 172.20.255.1, dst 10.255.255.1)
 ...
 → 활성 C8000V가 수신, GRE 캡슐 해제
```

이 마지막 홉(tgw-rtb의 `/32` 경로)이 **애니캐스트 설계에서 트래픽 방향을 결정하는 핵심 지점**입니다 —
...
- **eBGP 65000↔65200** (+BFD 500ms×3, hold 3/9): CGW가 `ONPREM-LAN`(172.20.51.0/24)만 광고
  ...
  CGW의 `TUNNEL-IN`은 VPC CIDR와 애니캐스트 /32의 유입을 차단합니다(**GRE 재귀
  라우팅 방지** — 터널 목적지로 가는 경로가 터널 자신을 가리키면 터널이 다운됩니다)
- **PIM sparse-mode**: RP = CGW Lo0. C8000V는 `ip mroute`로 RP와 소스의 RPF를
  터널로 강제합니다(유니캐스트 경로는 언더레이를 가리키는데, 언더레이에는 PIM이
  없기 때문입니다). Gi2의 `igmp static-group`이 만든 (*,G) 조인이 터널을 타고
  RP로 올라가고, 소스 트래픽이 터널로 내려와 Gi2 → TGW 도메인으로 송출됩니다
...
- Gi1은 DHCP 유지(ENI 고정 IP를 그대로 받음) — 정적으로 다시 설정해도 얻는 것 없이 세션이 끊길 위험만 생깁니다.
...
- 조인 앵커는 위의 socat 또는 [receiver-setup.txt](receiver-setup.txt)의 iperf2 서버
  (`-B 239.1.1.1` 조인) 중 하나면 됩니다 (둘 다 그룹 조인을 유지하는 역할).
- **iperf3는 멀티캐스트를 지원하지 않습니다** — 스트림 테스트는 온프렘에서 iperf2로 보냅니다(5.5 참조).
- 접속: 키페어 없이 `aws ec2-instance-connect ssh --instance-id <id> --connection-type eice`

---

## 4. CloudFormation 배포 자원

### 4.1 VPC 서브넷 (VPC 10.1.1.0/24, 서비스별 연속 배치)
...
> 멀티캐스트 도메인에는 어태치먼트에 속하지 않은 서브넷도 연결할 수 있습니다
> (실환경에서 검증). 그래서 어태치먼트는 전용 서브넷에, 도메인은 리시버
> 서브넷에 두는 분리 구성이 가능합니다.
...
| 10.1.1.90 | C8000V-1 Gi2 (멀티캐스트 송출) | ✅ 고정 (파라미터) |
| 10.1.1.100 | C8000V-2 Gi2 (멀티캐스트 송출) | ✅ 고정 (파라미터) |
| 10.255.255.1 | C8000V 공유 애니캐스트 Lo0 (GRE 앵커, RS가 활성 라우터 쪽으로 경로 지정) | ✅ 고정 (라우터 설정) |
...
### 4.4 타이머 설계 — 정상 종료부터 갑작스러운 장애까지

6.1에서 실측한 인스턴스 중지는 IOS-XE가 BGP를 정상 종료(graceful close)한 뒤에
내려가는, 말하자면 예고를 남기는 장애입니다. 반면 커널 패닉, 하이퍼바이저 장애,
인스턴스 강제 종료처럼 아무 신호 없이 멈추는 장애는 아래 BFD/BGP 타이머들이
감지를 맡습니다.

| 타이머 | 값 | 감지 시간 | 담당 상황 |
|---|---|---|---|
| RS BFD (Gi1) | 300ms × 3 | ~0.9초 | **활성 라우터의 갑작스러운 다운** — RS가 경로를 철회해 페일오버를 트리거하는 핵심 타이머 |
| 오버레이 BFD (Tu100) | 500ms × 3 | ~1.5초 | CGW가 죽은 라우터와의 오버레이 세션을 홀드타임 전에 정리 — 리턴 경로 전환 가속 |
| 오버레이 BGP hold (CGW, `timers 3 9`) | keepalive 3초 / hold 9초 | ≤9초 | 페일백 안전망 — 애니캐스트 특성상 BFD가 살아있는 다른 라우터의 응답에 혼동될 때 스테일 세션을 홀드타임으로 정리 |
| BGP `timers connect 5` (C8000V) | 5초 | — | 전환 직후 새 오버레이 세션의 재시도 간격 단축 (기본 30초 대기 제거) |
| RS Persist Routes | 2분 | — | 피어 플랩 시 마지막 경로 유지 — 일시 플랩이 블랙홀로 번지는 것 방지 |

- **갑자기 멈추는 장애라면**: RS BFD 감지(0.9초)에 경로 교체 시간을 더해 순방향
  기준 **약 1초의 단절**이 예상됩니다(단방향 스트림 기준). 리턴 경로는 대기
  라우터의 오버레이 BGP가 수립될 때까지 몇 초가 더 걸립니다.
- **정상 종료라면**: BGP notification이 BFD 만료보다 먼저 도착해
  make-before-break가 성립합니다 — 6.1의 iperf2 실측에서 무손실로 확인됐습니다.
- **더 조이지 않는 이유**: BFD를 100ms×3처럼 조이면 감지는 빨라지지만,
  DX를 건너는 오버레이 BFD는 경로 지터에 민감해져 오탐 플랩 위험이 커집니다.
  직결 구간(Gi1)은 300ms, 터널 구간은 500ms로 차등을 둔 것이 균형점입니다.
...
- **ttl=125** = 소스 기준 3홉(CGW → C8000V → 리시버, 128−3). 경로가 설계보다 길거나
  짧으면 이 값이 달라집니다.
- **첫 응답만 RTT가 큼(33ms → 이후 4~5ms)** = RP-tree 경유 후 SPT 전환. (S,G) 트리가
...
### 5.5 종단 테스트 (iperf2 UDP 스트림 — 실제 워크로드형)
...
tail -f /home/ec2-user/iperf-recv.log        # 백그라운드 기동 시
```

iperf2 서버 리포트 자체가 손실 측정 도구 역할을 합니다 — 매 인터벌마다
`Lost/Total Datagrams` 열이 찍히므로(초당 109패킷 = 해상도 ~9ms) 절체 순간의
손실이 0인지 몇 개인지 별도 캡처 없이 확인할 수 있습니다. 스트림이 끝나면
전체 요약(총 datagram 수, 손실률, 평균 지터)도 출력됩니다.

수신 공백을 밀리초 단위로 더 정밀하게 봐야 한다면 tcpdump 타임스탬프를
함께 기록해 두었다가 최대 수신 간격을 계산하면 됩니다:
...
### 6.1 C8000V 라우터 장애 (실측 완료)

절차: 온프렘 iperf2 클라이언트에서 UDP 멀티캐스트 스트림(5.5의
`iperf -c 239.1.1.1 -u -T 8 -b 1M -l 1200`, 초당 109패킷)을 흘리는 상태에서
활성 C8000V 인스턴스를 **중지했다가 재시작**합니다. 그동안 리시버 2대의 iperf2
서버(`iperf -s -u -B 239.1.1.1 -i 1`)가 손실과 지터를 초 단위로 기록합니다.

동작 순서: RS BFD 감지(~1초) → RIB 철회 → prepend된 C8000V-2 경로가 best 승격 →
tgw-rtb/receiver-rtb 넥스트홉 교체 → GRE가 대기 라우터로 재앵커(CGW 무변경) →
오버레이 BGP 재수립 → 부팅 완료 후 자동 선점으로 원복.

#### 실측 타임라인 (2026-07-12, iperf2 스트림 중 stop→start, 2사이클)

| 경과 | 이벤트 | 리시버 iperf2 리포트 |
|---|---|---|
| 0초 | 활성 C8000V-1 stop 명령 | 정상 수신 지속 |
| +2초 (1차) / +18초 (2차) | **페일오버**: RS가 앵커 /32 넥스트홉을 C8000V-2 Gi1로 교체 | **손실 0** — 지터만 ~1ms 스파이크 (평시 ~0.1ms) |
| +49초 | C8000V-1 start 명령 | 정상 (C8000V-2 경유) |
| +3분 27초 | C8000V-1 부팅 완료, RS 피어 BGP/BFD 재수립 | 정상 |
| +4분 41초 | **페일백**(자동 선점): 넥스트홉 → C8000V-1 복귀 | **손실 0** — 지터만 ~2ms 스파이크 |

- 페일오버 때 라우트가 교체되기까지 걸린 시간이 사이클마다 달랐던 것(+2초 vs
  +18초)은 IOS-XE 셧다운 진행 타이밍의 편차일 뿐, 어느 사이클에서도 수신
  공백은 없었습니다.
- 600초 스트림 통계(리시버 2대 동일): 1차 사이클 스트림 1/65,538(0.0015%,
  시작 1초차 1건 — 절체와 무관), 2차 사이클 스트림 **0/65,538 (0%)**.

#### 리시버 원본 로그 (발췌, 2차 사이클 스트림)

리시버(az1)의 iperf2 서버 로그 원문입니다. 절체 순간이 지터 스파이크로만
나타나고 `Lost/Total` 열이 끝까지 0인 것을 그대로 볼 수 있습니다.

페일오버 구간 — 지터가 0.1ms대에서 0.95ms로 잠깐 튀었을 뿐 손실은 없습니다:

```
[  2] 83.00-84.00 sec   128 KBytes  1.05 Mbits/sec   0.109 ms 0/109 (0%)
[  2] 84.00-85.00 sec   129 KBytes  1.06 Mbits/sec   0.950 ms 0/110 (0%)   <- 페일오버
[  2] 85.00-86.00 sec   128 KBytes  1.05 Mbits/sec   0.153 ms 0/109 (0%)
```

페일백 구간 — 선점 전환도 지터 스파이크 몇 개로만 나타나고 손실은 없습니다:

```
[  2] 350.00-351.00 sec   128 KBytes  1.05 Mbits/sec   0.745 ms 0/109 (0%)  <- 페일백 시작
...
[  2] 356.00-357.00 sec   128 KBytes  1.05 Mbits/sec   2.013 ms 0/109 (0%)  <- 복귀 완료
```

600초 스트림 최종 요약 — 페일오버와 페일백을 모두 포함한 전 구간에서 손실 0:

```
[  2] 0.00-600.00 sec  75.0 MBytes  1.05 Mbits/sec   0.075 ms 0/65538 (0%)
```

리시버(az2)도 동일한 요약(`0/65538 (0%)`)을 기록했습니다 — TGW 도메인이
같은 스트림을 두 AZ로 복제하면서 어느 쪽도 손실이 없었다는 뜻입니다.

#### 핵심 관찰

- **장애와 복귀 양방향 모두 make-before-break가 성립해 무손실입니다.**
  중지할 때는 IOS-XE가 BGP를 먼저 정상 종료(graceful close)하므로 데이터플레인이
  멈추기 전에 경로가 옮겨 가고, 복귀할 때는 라우터가 완전히 부팅해 PIM과
  static-group까지 준비를 마친 뒤에야 BGP가 올라와 경로가 되돌아오기 때문입니다.
- **순방향 멀티캐스트는 오버레이 BGP 재수렴과 무관합니다** — Gi2의
  `igmp static-group`이 미리 만들어 둔 (*,G)와 정적 mroute RPF 덕에 경로가
  바뀌는 즉시 트래픽이 흐릅니다. 시세·영상 피드 같은 단방향 배포형 워크로드는
  전 과정에서 손실이 없습니다.
- 커널 패닉이나 강제 종료처럼 **BGP를 정상 종료하지 못하고 갑자기 멈추는
  장애**라면, 수신 공백은 길어야 RS BFD가 장애를 감지하는 시간(~1초)입니다
  (4.4 타이머 설계 참고).
- 복구 후 원상 복귀는 자동입니다(선점) — C8000V-1이 부팅하면 별도 작업 없이
  활성으로 돌아오고, 복귀 중에도 손실이 없으므로 운영자가 개입할 일이 없습니다.
- C8000V가 부팅해 RS BGP를 다시 수립하기까지는 **약 3분~3분 30초**가 걸립니다.

### 6.2 Direct Connect 장애 (Bring down BGP)
...
> 물리 단선 시나리오는 VIF 서브인터페이스 shutdown(`interface Gi1.351` → `shutdown`)으로 재현할 수 있습니다.
...
| 증상 | 확인 | 원인/조치 |
|---|---|---|
| 터널 down/flap | `show interface Tunnel100`, `show ip route 10.255.255.1` | 재귀 라우팅 여부 — 터널로 10.255.255.1이나 10.1.1.0/24를 학습하면 안 됨(TUNNEL-IN 확인). DXGW 허용 프리픽스에 /32 포함 여부 |
...
실제 출력입니다. 운영 중 상태를 점검할 때 아래와 비교하세요 — 특히 **대기 라우터의
"오버레이 BGP Active(미수립)"는 장애가 아니라 정상 상태**라는 점을 놓치기 쉽습니다.
...
`show ip mroute 239.1.1.1 count` — 포워딩 카운터로 트래픽 통과를 정량적으로 확인합니다.
ping 1초 간격이면 초당 약 1패킷(아래 `52/1/122/0`의 두 번째 필드), RPF failed와
드롭은 0이어야 합니다. 트래픽이 끊기면 (S,G)는 약 3분 뒤 만료되고 카운터가
...
CGW에서 학습한 LAN을 재광고합니다 (활성 라우터만 LAN 경로를 가짐)
...
`show ip bgp summary` — **대기 라우터에서는 오버레이 세션이 Active(미수립)로 보이는
것이 정상**입니다. CGW의 GRE는 RS가 활성으로 정한 라우터에만 앵커되므로, 대기
라우터의 터널로는 CGW 트래픽이 오지 않아 오버레이 BGP가 수립되지 않습니다.
...
`show ip bgp neighbors 10.1.1.41 advertised-routes` — 애니캐스트 /32만 광고합니다
(AS-path prepend는 아웃바운드 정책이라 이 출력에는 보이지 않고, RS 라우팅 DB에서
AS×4로 확인됩니다). LAN 172.20.51.0/24는 오버레이 세션이 없어 광고되지 않는
것이 정상입니다.
...
`show ip mroute 239.1.1.1` — (*,G)는 있지만 RPF 네이버가 0.0.0.0입니다(터널 너머에
PIM 네이버가 없는 상태). 페일오버로 오버레이가 수립되면 활성 라우터와 같은
형태로 바뀝니다.
...
- [Transit Gateway 멀티캐스트 개요](https://docs.aws.amazon.com/vpc/latest/tgw/tgw-multicast-overview.html) — IGMPv2 쿼리 주기(2분), SG/NACL 요건(프로토콜 2, 소스 0.0.0.0/32) 등 이 문서 7장의 근거
- [멀티캐스트 도메인](https://docs.aws.amazon.com/vpc/latest/tgw/multicast-domains-about.html)
- [Transit Gateway 할당량 (MTU 포함)](https://docs.aws.amazon.com/vpc/latest/tgw/transit-gateway-quotas.html) — **멀티캐스트 패킷 단편화 미지원(드롭)**: 7.2의 GRE MTU 함정과 직결
...
- [허용 프리픽스(Allowed prefixes) 동작](https://docs.aws.amazon.com/directconnect/latest/UserGuide/allowed-to-prefixes.html) — 3.1의 수동 작업 ②의 근거
